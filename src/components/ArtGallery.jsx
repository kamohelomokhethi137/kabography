// Full upgraded ArtGallery.jsx with size options per image, responsive, secure, animated
            ...
            >
              {categories.map(cat => (
                <button
                  ...
                  className={`px-4 py-2 text-sm rounded-md transition font-medium hover:bg-black hover:text-white ${filter === cat ? 'bg-black text-white' : 'bg-white text-black border'}`}
                >
                  {cat}
                </button>
              ))}
        ...
        <div className="hidden sm:flex flex-wrap gap-2 justify-center">
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-2 text-sm rounded-full transition font-medium shadow-sm hover:bg-black hover:text-white ${filter === cat ? 'bg-black text-white' : 'bg-white text-black border'}`}
            >
              {cat}
            </button>
          ))}
            ...
            >
              ...
              <div className="p-6">
                ...
                <div className="mt-3 text-black">
                  <label className="block text-sm font-medium text-black mb-1">Select Size:</label>
                  <select
                ...
                <div key={item.cartKey} className="flex items-center gap-4 mb-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <button onClick={() => updateQuantity(item.cartKey, item.quantity - 1)} className="px-2 bg-gray-200 rounded text-black">-</button>
                      <span className="text-sm text-black">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.cartKey, item.quantity + 1)} className="px-2 bg-gray-200 rounded text-black">+</button>
                    </div>
                  </div>
                  <button onClick={() => removeFromCart(item.cartKey)} className="text-gray-400 hover:text-red-600">
                    <FiX />
                  </button>
                </div>
              ))}
